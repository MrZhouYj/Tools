...
@implementation CAFMDBModel

+ (BOOL)createTable
{
    //拿到数据库路径，创建数据库
    FMDatabase *db = [FMDatabase databaseWithPath:[CAFMDBDatabase dbPath]];
    //开启数据库
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
...
- (BOOL)save
{
    ...
    CAFMDBDatabase *ykDB = [CAFMDBDatabase shareFMDBDatabase];
    __block BOOL res = NO;
...
{
    //判断是否是CAFMDBModel的子类
    for (CAFMDBModel *model in array) {
        if (![model isKindOfClass:[CAFMDBModel class]]) {
            return NO;
        }
    }
    
    __block BOOL res = YES;
    CAFMDBDatabase *jkDB = [CAFMDBDatabase shareFMDBDatabase];
    // 如果要支持事务
...
{
    //如果数组中的模型对象不是CAFMDBModel的子类，那么不好意思，处理不了
    for (CAFMDBModel *model in array) {
        if (![model isKindOfClass:[CAFMDBModel class]]) {
            return NO;
        }
...
+ (NSArray *)findByCriteria:(NSString *)criteria
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            CAFMDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
