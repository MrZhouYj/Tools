...
#import "CAAgreementViewController.h"
...
-(void)is_off_market{
    ...
    [CANetworkHelper GET:CAAPI_CRYPTO_TO_CRYPTO_IS_OFF_MARKET_DATE  parameters:nil success:^(id responseObject) {
...
-(void)cancleEntrust:(CAEntrustModel *)model{
    ...
    [CAAlertView showAlertWithTitle:CALanguages(@"您确定取消当前委托吗？")  message:@"" completionBlock:^(NSUInteger buttonIndex, CAAlertView * _Nonnull alertView) {
        if (buttonIndex==1) {
            ...
            [SVProgressHUD show];
            [CANetworkHelper MATCHERPOST:[NSString stringWithFormat:@"%@%@",self.market_id,CAAPI_CRYPTO_TO_CRYPTO_CANCEL_ORDERS] parameters:para success:^(id responseObject) {
                [SVProgressHUD dismiss];
                if ([responseObject[@"code"] integerValue]==20000) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [weakself getRatios];
...
-(void)cancleAllEntrust{
    ...
    if (buttonIndex==1) {
            ...
            [SVProgressHUD show];
            [CANetworkHelper MATCHERPOST:[NSString stringWithFormat:@"%@%@",self.market_id,CAAPI_CRYPTO_TO_CRYPTO_CANCEL_ALL_ORDERS] parameters:para success:^(id responseObject) {
                [SVProgressHUD dismiss];
                if ([responseObject[@"code"] integerValue]==20000) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [weakself getRatios];
...
-(void)gotoAgentController{
    CAAgreementViewController * agree = [CAAgreementViewController new];
