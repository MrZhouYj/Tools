...
-(CAMineHeaderView *)headView{
    if (!_headView) {
        _headView = [[CAMineHeaderView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame),150+kStatusBarHeight)];
    }
    return _headView;
}
...
-(NSArray *)dataArray{
    if (!_dataArray) {
        NSString * agent_c = @"CAAgreementViewController";
        CAUser * user = [CAUser currentUser];
        if (user.is_agent) {
            agent_c = @"CAAgentViewController";
        }
        
        _dataArray = @[
            @{@"image":@"wallet",@"text":@"我的资产",@"controller":@"CAMoneyViewController",@"isNeesLogin":@(YES)},
            @{@"image":@"transfer",@"text":@"资金划转",@"controller":@"CATransferViewController",@"isNeesLogin":@(YES)},
