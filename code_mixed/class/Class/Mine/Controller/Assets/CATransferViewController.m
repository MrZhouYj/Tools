...
@property (nonatomic, strong) CAInputView * amountTf;
@property (nonatomic, strong) NSArray * currencyModels;
@property (nonatomic, strong) NSArray * currencyArray;
@property (nonatomic, strong) NSArray * accounts;

@property (nonatomic, strong) CACurrencyMoneyModel * currentAccount;
...
-(NSArray *)accounts{
    if (!_accounts) {
        _accounts = [[CAAccounts shareAccouts] getAccounts];
    }
    return _accounts;
}

-(NSArray *)currencyArray{
    _currencyArray = [CACurrencyMoneyModel getCodeBigArray:self.currencyModels];
...
-(void)getAsset{
    ...
    [CANetworkHelper GET:CAAPI_MINE_ASSET parameters:para success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue]==20000) {
            ...
            self.currencyModels = [CACurrencyMoneyModel getModels:data[@"list"]];
            [self changeCode];
        }else{
...
-(void)transferAction{
    ...
    NSDictionary * para = @{
     ...
     [CANetworkHelper POST:CAAPI_MINE_ACCOUNT_TRANSFER parameters:para success:^(id responseObject) {
         [SVProgressHUD dismiss];
         Toast(responseObject[@"message"]);
         if ([responseObject[@"code"] integerValue]==20000) {
...
-(void)changeCode{
    ...
    if (self.code.length) {
        for (NSInteger i=0; i<self.currencyModels.count; i++) {
            CACurrencyMoneyModel * model  =self.currencyModels[i];
...
-(void)setCurrentAccount:(CACurrencyMoneyModel *)currentAccount{
...
-(void)setupUI{
    ...
    self.amountTf = [CAInputView showLoginTypeInputView];
    ...
    self.saveButton = [CABaseButton buttonWithTitle:@"划转"];
