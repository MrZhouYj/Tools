...
#import "TAMomentsCustoms.h"
#import "TAFeelsMedical.h"
...
@property (nonatomic, strong) TAMomentsCustoms * headView;
...
-(void)getData{
    ...
    [TACrimeStudy GET:CAAPI_MINE_ASSETS_RECORDS parameters:para success:^(id responseObject) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TAFeelsMedical *cell = [tableView dequeueReusableCellWithIdentifier:@"TAFeelsMedical"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row<self.dataArray.count) {
        TAWanderShopping * record = [TAWanderShopping new];
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[TAFeelsMedical class] forCellReuseIdentifier:@"TAFeelsMedical"];
        kWeakSelf(self);
...
-(TAMomentsCustoms *)headView{
    if (!_headView) {
        _headView = [[TAMomentsCustoms alloc] initWithFrame:CGRectMake(0, 0, MainWidth, 278+KTopRevise)];
...
//        if ([contro isKindOfClass:[CARechargeViewController class]]||[contro isKindOfClass:[CAWithDrawViewController class]]||[contro isKindOfClass:[TAChancesRemarks class]]) {
