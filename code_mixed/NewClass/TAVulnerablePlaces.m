...
@implementation TAVulnerablePlaces

+(instancetype)shareSocket{
    static TAVulnerablePlaces * socket = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        socket = [TAVulnerablePlaces new];
               ...
               if (weakself.ws.readyState==SR_OPEN) {
                   if (weakself.waitSendDataArray.count>0) {
                       @try {
                           ...
                           if (weakself.waitSendDataArray&&weakself.waitSendDataArray.count>0) {
...
//        [mutDictionary setValue:NSStringFormat(@"%@·%@",mutDictionary[@"subscribe"],[TAUrbanCooperation language]) forKey:@"subscribe"];
//    }
