...
#import "TAAlwaysPolitical.h"
#import "TAHappyXigaze.h"
#import "TALeavingThere.h"
#import "TAHappinessAbsorbing.h"
#import "TAOthersGardens.h"
#import "TAThingsRegion.h"
#import "TAFrontFriends.h"
#import "TANecessarilyAnimal.h"
#import "TAHurtsSmiling.h"
#import "TAEverythingHostels.h"
#import "TASweetWatching.h"
#import "TAYourselfProven.h"

@interface TAWhereScenic ()
<UITableViewDelegate,
...
@property (nonatomic, strong) TAAlwaysPolitical * topView;
@property (nonatomic, strong) TAHappyXigaze * bottomView;
@property (nonatomic, strong) TALeavingThere * chatView;
...
-(void)setPayNotiLabelText{
    ...
    if (_payTypeString.length&&[TAAppreciatePlans currentUser].real_name.length) {
        NSString * langStr = CALanguages(@"请使用本人((%@))[%@]向以下账户自行转账");
        NSString * member_real_name = [TAAppreciatePlans currentUser].real_name;
...
-(void)TAHurtsSmiling_didSelected:(NSString *)payType{
    ...
    TASweetWatching * model1 = [TASweetWatching new];
    TASweetWatching * model2 = [TASweetWatching new];
    TASweetWatching * model3 = [TASweetWatching new];
    ...
    }else if ([payType isEqualToString:Wechat_pay]){
    ...
    }else if ([payType isEqualToString:Bank_card_pay]){
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[TANecessarilyAnimal class] forCellReuseIdentifier:@"TANecessarilyAnimal"];
        [_tableView registerClass:[TAEverythingHostels class] forCellReuseIdentifier:@"TAEverythingHostels"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        TAEverythingHostels * cell = [tableView dequeueReusableCellWithIdentifier:@"TAEverythingHostels" forIndexPath:indexPath];
        cell.orderInfoModel = self.orderInfoModel;
        return cell;
    }else if (indexPath.section==1){
        
        TANecessarilyAnimal * cell = [tableView dequeueReusableCellWithIdentifier:@"TANecessarilyAnimal" forIndexPath:indexPath];
        if (indexPath.row<self.payLists.count) {
            TASweetWatching * model = self.payLists[indexPath.row];
            ...
            cell.block = ^(){
                ...
                TAYourselfProven *popView = [TAYourselfProven new];
        ...
        if (self.detailLists.count>indexPath.row) {
            TASweetWatching * model = self.detailLists[indexPath.row];
            cell.leftTitle = model.leftTitle;
            cell.rightContentText = model.rightContentText;
            cell.enableCopy = model.enableCopy;
        }
...
-(void)dealDetailsData{
    ...
    //昵称
    [mutArray addObject:[TASweetWatching modelWithleftTitle:self.orderInfoModel.notiShowNikeName rightContent:self.orderInfoModel.showNickName showRow:NO enableCopy:NO]];
    //实名
    [mutArray addObject:[TASweetWatching modelWithleftTitle:self.orderInfoModel.notiShowRealName rightContent:self.orderInfoModel.showRealName showRow:NO enableCopy:NO]];
    //订单号
    [mutArray addObject:[TASweetWatching modelWithleftTitle:@"订单号" rightContent:self.orderInfoModel.sn showRow:NO enableCopy:YES]];
    //订单时间
    [mutArray addObject:[TASweetWatching modelWithleftTitle:@"订单时间" rightContent:self.orderInfoModel.created_at showRow:NO enableCopy:NO]];

    if (self.orderInfoModel.payment.length) {
        [mutArray addObject:[TASweetWatching modelWithleftTitle:_orderInfoModel.notiPay rightContent:self.orderInfoModel.payment showRow:NO enableCopy:NO]];
    }
    if (self.orderInfoModel.dispute_status.length) {
        [mutArray addObject:[TASweetWatching modelWithleftTitle:@"申诉进度" rightContent:self.orderInfoModel.dispute_status showRow:NO enableCopy:NO]];
    }
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([[TASuburbsLives getCurrentSkinType] isEqualToString:DKThemeVersionNormal]) {
...
- (void)viewDidLoad {
    ...
    self.orderInfoModel = [TAHappinessAbsorbing new];
    [SVProgressHUD show];
...
-(void)getData{
    ...
    [TACrimeStudy closeLog];
    [TACrimeStudy GET:CAAPI_OTC_SHOW_TRADING parameters:para success:^(id responseObject) {
        //刷新数据
...
-(TALeavingThere *)chatView{
    if (!_chatView) {
        _chatView = [[TALeavingThere alloc] initWithFrame:CGRectMake(0, kTopHeight, MainWidth, MainHeight-kTopHeight)];
...
-(TAHappyXigaze *)bottomView{
    if (!_bottomView) {
        _bottomView = [TAHappyXigaze new];
        [self.view addSubview:_bottomView];
    }
    return _bottomView;
}

-(TAAlwaysPolitical *)topView{
    if (!_topView) {
        _topView = [TAAlwaysPolitical new];
    ...
    if ([eventName isEqualToString:@"order_action"]) {
        ...
        switch ([userInfo integerValue]) {
            case CALogalOrderActionCancle:{
                [[TAOthersGardens new] showAlert:@"" title:@"确认取消交易" subTitle:@"如果您已经向卖家付款，请千万不要取消交易" notiTitle:@"取消规则：当日累计取消法币订单超过3次,会限制当日法币交易功能" confirmBlock:^(id idex){
            ...
            {
                [[TAOthersGardens new] showAlertTitle:@"付款确认" subTitle:@"请选择您的支付方式" importPayMethods:self.orderInfoModel.supportPayMethods confirmBlock:^(NSString * payMethod) {
            ...
            case CALogalOrderActionExpendTime:
                {
                    [[TAOthersGardens new] showAlert:@"" title:@"是否延长时间" subTitle:@"将延长15分钟时间" notiTitle:@"" confirmBlock:^(id idex){
            ...
            case CALogalOrderActionReleaseCoin:
                {
                   [[TAOthersGardens new] showAlert:@"" title:@"确认放行" subTitle:@"请务必登录网上银行或第三方支付账号确认收到该笔款项" notiTitle:@"" confirmBlock:^(id idex){
        ...
        if (NSStringFormat(@"%@",self.orderInfoModel.ID).length) {
            TAThingsRegion * appeal = [TAThingsRegion new];
...
-(void)dealActionRequest_cancle{
    ...
    [TACrimeStudy POST:CAAPI_OTC_CANCLE_TRADING parameters:para success:^(id responseObject) {
        //刷新数据
        [SVProgressHUD dismiss];
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([responseObject[@"code"] integerValue]==20000) {
...
-(void)dealActionRequest_hasPaied:(NSString*)payMethod{
      ...
      [SVProgressHUD show];
       ...
       [TACrimeStudy POST:CAAPI_OTC_MARK_BUYER_HAS_PAID parameters:para success:^(id responseObject) {
           //刷新数据
           [SVProgressHUD dismiss];
           dispatch_async(dispatch_get_main_queue(), ^{
               if ([responseObject[@"code"] integerValue]==20000) {
...
-(void)dealActionRequest_expendTime{
    ...
    [SVProgressHUD show];
     ...
     [TACrimeStudy POST:CAAPI_OTC_MARK_TRADING_OVERTIME parameters:para success:^(id responseObject) {
         //刷新数据
         [SVProgressHUD dismiss];
         dispatch_async(dispatch_get_main_queue(), ^{
             if ([responseObject[@"code"] integerValue]==20000) {
...
-(void)dealActionRequest_releaseCoin{
    [SVProgressHUD show];
     ...
     [TACrimeStudy POST:CAAPI_OTC_SELLER_HAS_RELEASED parameters:para success:^(id responseObject) {
         //刷新数据
         [SVProgressHUD dismiss];
         dispatch_async(dispatch_get_main_queue(), ^{
             if ([responseObject[@"code"] integerValue]==20000) {
