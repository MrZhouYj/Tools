...
#import "LQXSwitch.h"
#import "TAOtherwiseExperience.h"
#import "TAAppreciatePlans.h"

@interface TAThereComplex()
{
    TAOtherwiseExperience * _menuView;
}
...
@property (nonatomic, strong) TAAppreciatePlans * user;
@end

@implementation TAThereComplex
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self initSubViews];
        self.user = [TAAppreciatePlans currentUser];
        [self addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(event_gotoUserInfo)]];
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userDidChangeInfomation) name:TAAppreciatePlansDidChangedUserInfomationNotifacation object:nil];
...
-(void)languageDidChange{
    ...
    self.switchView.onText = CALanguages(@"夜间");
    self.switchView.offText = CALanguages(@"白天");
...
//    _menuView = [TAOtherwiseExperience new];
...
-(UILabel *)nameLabel{
    if (!_nameLabel) {
        ...
        _nameLabel.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(LQXSwitch *)switchView{
    if (!_switchView) {
        ...
        _switchView.changeBlock = ^(BOOL ison){
            ...
            if (ison) {
                //设置为黑夜模式
                [TASuburbsLives setSkin:DKThemeVersionNight];
            }else{
                //设置为白天模式
                [TASuburbsLives setSkin:DKThemeVersionNormal];
            }
        };
...
-(void)event_gotoUserInfo{
    ...
    if (_user.isAvaliable) {
        [self routerEventWithName:@"pushViewController" userInfo:@"TAStolenUsually"];
    }else{
        [self routerEventWithName:@"pushViewController" userInfo:@"TAAlongDuring"];
    }
}
