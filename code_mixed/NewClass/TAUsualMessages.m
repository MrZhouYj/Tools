...
@interface TAUsualMessages()
<TAShowingIndeedDelegata,
UITextFieldDelegate>
{
    TAShowingIndeed * _unitLastView;
    TAShowingIndeed * _stateLastView;
}
...
@implementation TAUsualMessages
...
-(void)setIsHistory:(BOOL)isHistory{
    ...
    if (isHistory) {
          ...
          array = @[CALanguages(@"买入"),CALanguages(@"出售")];
    }
...
-(void)addSubViews:(NSArray*)array to:(UIView*)spView baseTag:(NSInteger)baseTag{
    ...
    for (int i=0; i<array.count; i++) {
        TAShowingIndeed * row = [TAShowingIndeed new];
...
-(TAShowingIndeed *)rowView{
    if (!_rowView) {
        TAShowingIndeed * row = [TAShowingIndeed new];
        [self addSubview:row];
        _rowView = row;
        row.layOut = TAShowingIndeedLayoutBetween;
