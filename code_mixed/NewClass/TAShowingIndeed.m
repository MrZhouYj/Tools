...
@implementation TAShowingIndeed
...
-(void)didChange{
    ...
    if (self.delegata && [self.delegata respondsToSelector:@selector(TAShowingIndeed_didChangeRowState:rowView:)]) {
        
        [self.rowBtn setSelected:!self.up];
        
        [self.delegata TAShowingIndeed_didChangeRowState:!self.up rowView:self];
...
-(void)layoutSubviews{
    ...
    switch (self.layOut) {
        case TAShowingIndeedLayoutCenter:
        {
            ...
            break;
        case TAShowingIndeedLayoutBetween:
        {
            [self.contentView mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)setLayOut:(TAShowingIndeedLayout)layOut{
    _layOut = layOut;
    [self setNeedsLayout];
}
