...
@implementation TAGoodsEvening

- (void)viewDidLoad {
    [super viewDidLoad];
    
    TAAppreciatePlans * currentUser = [TAAppreciatePlans currentUser];
...
-(void)chooseAction{
    ...
    [TAAreasCalled showActionSheet:self.agents key:@1 selectIndex:_index click:^(id  _Nonnull value, NSInteger index) {
...
-(void)getData{
    ...
    [SVProgressHUD show];
    [TACrimeStudy GET:CAAPI_MINE_GET_AGENT_DATA parameters:nil success:^(id responseObject) {
...
-(void)save{
    ...
    [SVProgressHUD show];
    [TACrimeStudy POST:CAAPI_MINE_IMPROVE_INFORMATION parameters:para success:^(id responseObject) {
       
        [SVProgressHUD dismiss];
        [TAAppreciatePlans currentUser].have_agent_code_or_not = self->_agent_code;
