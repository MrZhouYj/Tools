...
@interface TALivesTogether()<UIScrollViewDelegate>
{
    UIButton * _lastBtn;
    CGFloat _itemWidth;
    CGRect _originRect;
...
@implementation TALivesTogether
...
-(UIButton*)private_creatButtonTitle:(NSString *)title tag:(NSInteger)tag{
    ...
    if (self.selectedColor) {
        ...
        [button setTitleColor:HexRGB(0x7540ee) forState:UIControlStateSelected];
    }
...
-(void)event_click:(UIButton*)btn{
    ...
    if (self.delegata&&[self.delegata respondsToSelector:@selector( TALivesTogether_didSelectedIndex:)]) {
        [self.delegata  TALivesTogether_didSelectedIndex:btn.tag-111];
    }
