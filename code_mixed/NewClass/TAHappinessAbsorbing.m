...
@implementation TAHappinessAbsorbing
...
-(void)initUIState{
    //   广告主： 创建当前广告的人； 创建订单用户：发起法币交易 点击购买出售 创建了一个订单的人
    //   1.广告主 进入订单详情
    //        1.1当前广告为买 广告主想买入币种 ad_trade_type==buy
    //            应该显示创建订单用户的支付信息 然后等待创建订单用户 放币 完成交易
    //        1.2当前广告为卖 广告主想出售自己的币种 ad_trade_type==sell 不用显示付款信息 等待付款成功 广告主放币 完成交易
    //   2.创建订单用户 进入到订单详情
    //        2.1当前订单为买 创建订单用户是想要买入币种 trade_type = buy 应该显示广告主的支付信息  创建订单用户支付成功 等待广告主放币 完成交易
    //        2.2当前订单为卖  创建订单用户是想出售币种 trade_type = sell 不用显示付款信息 等待广告主支付成功 创建订单用户放币 完成交易
    ...
    if (self.order_state==CAOrderStateWaitingPay||self.order_state==CAOrderStateHasExpendTime) {//待付款状态
        ...
        if ((self.is_builder && [self.ad_trade_type isEqualToString:@"sell"])||(!self.is_builder && [self.ad_trade_type isEqualToString:@"buy"])){
            isShowPayMenthod = NO;
