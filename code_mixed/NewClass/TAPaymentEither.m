...
@implementation TAPaymentEither
...
-(void)getData{
    ...
    [TACrimeStudy GET:CAAPI_MINE_NEW_PAYMENT_METHOD parameters:nil success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue]==20000) {
            ...
            for (NSString * str in data) {
                ...
                }else if ([str isEqualToString:Wechat_pay]){
                    [self.dataArray addObject:@{
                        @"logoImage":@"WeChat",
                        @"type":str,
                        @"text":@"微信",
                    }];
                }else if ([str isEqualToString:Bank_card_pay]){
                    [self.dataArray addObject:@{
                        @"logoImage":@"Bankcard",
                        @"type":str,
                        @"text":@"银行卡",
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row<self.dataArray.count) {
        ...
        cell.textLabel.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.dataArray.count>indexPath.row) {
        TATransformationStudy * view = [TATransformationStudy new];
...
-(void)tableViewHeaderView{
    ...
    titleLabel.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
