...
#import "TAGrowsEasily.h"
#import "TAFutureStrengthening.h"
#import "TABrightestTrain.h"
@interface Y_KLineMainView()
{
    BOOL _isIgnoreContentOffsetObserver;
}
/**
 *  需要绘制的model数组
 */
...
//    NSLog(@"开始绘制日期");
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.lineKTime == KLineTypeTimeLine || self.lineKTime == KLineType1Min) {
            formatter.dateFormat = @"HH:mm";
        }
        else if (self.lineKTime == KLineType1Day ||self.lineKTime == KLineType1Week || self.lineKTime == KLineType1Month ){
        ...
        if(CGPointEqualToPoint(lastDrawDatePoint, CGPointZero) || (point.x - lastDrawDatePoint.x > 60) ){
            if (self.isFullScreen) {
                CGRect rect = CGRectMake(drawDatePoint.x + 2, MainWidth - 98, 50, self.frame.size.height -Y_StockChartKLineMainViewMaxY);
                ...
                y = CGRectGetMaxY(self.parentScrollView.frame)-(self.frame.size.height -Y_StockChartKLineMainViewMaxY)-2;
                
                if (self.onlyShowTimeLineView) {
...
- (void)drawMaxAndMinValueAndCurrentPrice{
    ...
    if (lowIndex&&lowIndex>=self.needDrawKLinePositionModels.count) {
        return;
    }
...
//        y = ABS(maxY - (model.Close.doubleValue - low.doubleValue)/unitValue);
    ...
    CGFloat screenWidth = self.isFullScreen ? MainHeight :MainWidth;
    if (lastPointX.x > self.parentScrollView.frame.size.width) {
        ...
        //横屏实时价格的x坐标 减50（指标宽度）
        [self.currentPriceView setFrame:CGRectMake(self.isFullScreen?priceRectx-50-SafeAreaBottomHeight-SafeAreaTopHeight:priceRectx, y - 7.5, Y_StockChartKLinePriceViewWidth, 15)];
...
//    kLineViewWidth-=SafeAreaBottomHeight;
    ...
    if (!self.isDragging&&self.isNeedAutoScroll) {
        _isIgnoreContentOffsetObserver = YES;
        [self.parentScrollView setContentOffset:CGPointMake(sizeWidth-MainWidth, 0) animated:NO];
        _isIgnoreContentOffsetObserver = NO;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if([keyPath isEqualToString:Y_StockChartContentOffsetKey]&&!_isIgnoreContentOffsetObserver)
    {
