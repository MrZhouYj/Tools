...
@implementation TAHappiestAdvisor
...
-(void)initSubViews{
    
    {
      ...
      [self.headUserNameLabel dk_setBackgroundColorPicker:DKColorPickerWithKey(LogoColor)];
    ...
    //用户名
    {
        ...
        self.usernameLabel.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
    }
...
-(void)setModel:(TASorrowPress *)model{
    _model = model;
    ...
    if ([self.action_type isEqualToString:@"MyAdvertisementList"]) {
        ...
        if ([model.trade_type isEqualToString:@"sell"]) {
            ...
            [self.buyButton setTitle:NSStringFormat(@"%@ %@",CALanguages(@"出售"),model.code_big) forState:UIControlStateNormal];
    ...
    for (int i=0; i<array.count; i++) {
        ...
        if ([array[i] isEqualToString:Bank_card_pay]) {
            cartImageV.image = IMAGE_NAMED(@"Bankcard");
        }else if ([array[i] isEqualToString:Alipay]){
            cartImageV.image = IMAGE_NAMED(@"Alipay");
        }else if ([array[i] isEqualToString:Wechat_pay]){
