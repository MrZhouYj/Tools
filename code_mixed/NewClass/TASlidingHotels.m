...
@interface TASlidingHotels ()

@end

@implementation TASlidingHotels

- (void)viewDidLoad {
    
    [super viewDidLoad];
    
    self.view.dk_backgroundColorPicker = DKColorPickerWithKey(WhiteItemBackGroundColor);
...
-(TAPoliceCommittee *)navcBar{
    if (!_navcBar) {
        _navcBar = [[TAPoliceCommittee alloc] initWithFrame:CGRectMake(0, 0, MainWidth, kTopHeight)];
...
-(UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        CGFloat topSpace = kTopHeight;
        if (self.bigNavcTitle.length) {
            topSpace += 55;
        }
        [_scrollView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(self.view).insets(UIEdgeInsetsMake(topSpace, 0, 0, 0));
        }];
    }
    return _scrollView;
}
