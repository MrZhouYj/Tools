...
-(void)getData{
    ...
    [TACrimeStudy GET:CAAPI_COMMON_CONUNTRY_CODES parameters:nil success:^(id responseObject) {
        self.dataArray = [TATriedWiden getModels:responseObject[@"country_codes"]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TAComesSmiling_didSelected:)]) {
        
        [self.delegate TAComesSmiling_didSelected:self.dataArray[indexPath.row]];
        [self.navigationController popViewControllerAnimated:YES];
    }
...
-(void)searchWithKey:(NSString*)keyString{
    ...
    if (keyString&&keyString.length) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
           ...
           for (TATriedWiden *model in self.dataArray) {
