...
- (void)viewDidLoad {
    ...
    self.currentLanguage = [TAUrbanCooperation userLanguage];
...
-(void)dealIdCardDetectResult{
    ...
    NSLog(@"%@",para);
    __weak FaceidViewController* weakSelf = self;
    [TACrimeStudy uploadImagesWithURL:CAAPI_MINE_UPLOAD_ID_CARD_INFORMATION parameters:para name:@[@"front",@"back"] images:@[_detectItem.idcardPortraitImage,_detectItem.idcardEmblemImage] progress:^(NSProgress *progress) {
        ...
        if (deltaData&&error.errorType==MGFaceIDLiveDetectErrorNone) {
            ...
            [[MegNetwork singleton] queryDemoMGFaceIDAntiSpoofingVerifyWithBizToken:bizTokenStr verify:deltaData success:^(NSInteger statusCode, NSDictionary * _Nonnull responseObject) {
                ...
                if ([responseObject[@"result_code"] integerValue]==1000) {
                    ...
                    //                失败了
                    NSLog(@"不匹配 %ld----%@",(long)statusCode,responseObject[@"result_code"]);
...
-(void)dealMGFaceIDLiveDetectResult:(UIImage*)image{
    ...
    [TACrimeStudy uploadImagesWithURL:CAAPI_MINE_AFTER_SCAN_FACE parameters:nil name:@[@"man"] images:@[image] progress:^(NSProgress *progress) {
    } success:^(id responseObject) {
