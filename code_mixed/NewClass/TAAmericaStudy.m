...
+(NSArray*)getModels:(NSArray*)array{
    ...
    if (array.count) {
        ...
        for (int i=0; i<array.count; i++) {
            TAAmericaStudy * model = [TAAmericaStudy new];
            [model dealData:array[i]];
            [marketsArray addObject:model];
        }
        
        [TAAmericaStudy clearTable];
        [TAAmericaStudy saveObjects:marketsArray];
        
        return marketsArray;
    }else{
        return [TAAmericaStudy findAll];
    }
}
...
+ (NSArray *)getSymbolsBy:(NSString *)bid_unit{
    ...
    NSArray * localData = [TAAmericaStudy findByCriteria:sql];
...
+ (TAAmericaStudy *)getDefultSymbol{
    ...
    NSArray * localData = [TAAmericaStudy findByCriteria:sql];
    if (!localData.count) {
        localData = [TAAmericaStudy findAll];
    }
    if (localData.count) {
...
+(void)getMarketList:(TAAmericaStudyGetListSuccess)block{

    [TACrimeStudy GET:CAAPI_CRYPTO_TO_CRYPTO_GET_MARKETS parameters:nil success:^(id responseObject) {
        if ([responseObject[@"code"] integerValue]==20000) {
            ...
            if (markets.count) {
                [TAAmericaStudy getModels:markets];
            }
...
+ (NSArray *)getCurrenciesByCoinRegion:(NSString*)coinRegion{
    ...
    if (localData.count&&segments.count) {
        ...
        for (TAAmericaStudy * model in localData) {
...
+ (NSArray*)screenMarketsListWithCoinRegion:(NSString*)coinRegion currencyCode:(NSString*)code sortKey:(NSString*)sortKey sortType:(CASymbolsSortType)type{
    ...
    if ([coinRegion isEqualToString:@"favourite"]) {
        localData = [TAAmericaStudy getSymbolsFav];
    }else{
...
//        }
    ...
    if (localData.count&&type!=CASymbolsSortTypeNone) {
       localData = [localData sortedArrayUsingComparator:^NSComparisonResult(TAAmericaStudy*  _Nonnull obj1, TAAmericaStudy*  _Nonnull obj2) {
...
+(void)getAllFavouriteMarkets:(TAAmericaStudyGetListSuccess)block{
    
    [TACrimeStudy GET:CAAPI_CRYPTO_TO_CRYPTO_SHOW_ALL_FAVOURITE_MARKETS_DETAILS parameters:nil success:^(id responseObject) {
...
+(NSArray *)getSymbolsFav{
    ...
    if (localFav&&localFav.count) {
        ...
        for (NSString * market_id in localFav) {
            ...
            TAAmericaStudy * model = [TAAmericaStudy findFirstByCriteria:sql];
            if (model) {
                [mut addObject:model];
            }
        }
...
+ (NSArray *)getAllSymbols{
    ...
    NSArray * local = [TAAmericaStudy findAll];
    if (local) {
        return local;
    }
    return @[];
...
+ (NSArray *)getHomeSymbols{
    ...
    NSArray * local = [TAAmericaStudy findByCriteria:sql];
    if (local) {
        return local;
    }
    return @[];
...
+(NSArray *)getSymbolsSortByPriceChangeRatio{
    NSArray * localData = [TAAmericaStudy findAll];
    localData = [localData sortedArrayUsingComparator:^NSComparisonResult(TAAmericaStudy*  _Nonnull obj1, TAAmericaStudy*  _Nonnull obj2) {
...
+ (void)add_to_favorates:(NSString *)market_id finshed:(ActionResult)block{
    ...
    [TACrimeStudy POST:CAAPI_CRYPTO_TO_CRYPTO_ADD_TO_FAVAORATES parameters:para success:^(id responseObject) {
...
+ (void)remove_from_favorates:(NSString *)market_id finshed:(ActionResult)block{
    ...
    [TACrimeStudy POST:CAAPI_CRYPTO_TO_CRYPTO_REMOVE_FROM_FAVAORATES parameters:para success:^(id responseObject) {
        [SVProgressHUD dismiss];
        Toast(responseObject[@"message"]);
        if ([responseObject[@"code"] integerValue]==20000) {
