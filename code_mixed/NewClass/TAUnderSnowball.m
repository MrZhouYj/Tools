...
@implementation TAUnderSnowball
...
-(void)initSubViews{
    ...
    self.leftLabel.dk_textColorPicker = DKColorPickerWithKey(BoldTextColor);
    
    self.leftSmallLabel.font = FONT_REGULAR_SIZE(10);
    self.leftSmallLabel.dk_textColorPicker = DKColorPickerWithKey(SmallGrayTextColor);
    
    self.leftBottomLabel.font = FONT_REGULAR_SIZE(11);
    self.leftBottomLabel.dk_textColorPicker = DKColorPickerWithKey(SmallGrayTextColor);
    
    self.centerLabel.font = FONT_SEMOBOLD_SIZE(15);
    self.centerLabel.dk_textColorPicker = DKColorPickerWithKey(BoldTextColor);
    
    self.centerBottomLabel.font = FONT_REGULAR_SIZE(11);
    self.centerBottomLabel.dk_textColorPicker = DKColorPickerWithKey(SmallGrayTextColor);
...
-(void)setModel:(TAAmericaStudy *)model{
    _model = model;
...
//        self.leftSmallLabel.text = NSStringFormat(@"/%@",model.bid_unit);
        ...
        if (self.cellStyle==1) {
            ...
            self.rightLabel.text = NSStringFormat(@"%@",[TAAmericaStudy formatNumber:model.turnover]);
        }
        if (!self.showInHome) {
            self.leftBottomLabel.text = NSStringFormat(@"%@%@",CALanguages(@"成交额"),model.volume);
