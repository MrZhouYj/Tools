...
//    /mobile_api/v2/otc/edit_nick_name
    ...
    [TACrimeStudy GET:CAAPI_OTC_EDIT_NICK_NAME parameters:nil success:^(id responseObject) {
...
-(void)saveAction{
    ...
    [TACrimeStudy POST:CAAPI_OTC_UPDATE_NICK_NAME parameters:dic success:^(id responseObject) {
        [SVProgressHUD dismiss];
        Toast(responseObject[@"message"]);
        if ([ responseObject[@"code"] integerValue]==20000) {
            [[NSNotificationCenter defaultCenter] postNotificationName:TAAppreciatePlansDidChangedUserInfomationNotifacation object:nil];
