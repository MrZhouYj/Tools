...
#import "TABasedSouthwest.h"

@interface TABasedSouthwest()
...
@implementation TABasedSouthwest
...
-(void)eyeButtonClick:(UIButton*)button{
    if (button.isSelected) {
        if (button==self.mainEyeButton) {
            ...
            if (self.delegata&&[self.delegata respondsToSelector:@selector(TABasedSouthwest_didSelectedStatus:)]) {
                [self.delegata TABasedSouthwest_didSelectedStatus:Y_StockChartTargetLineStatusCloseMA];
            }
        }else{
            self.subSelButton = nil;
            if (self.delegata&&[self.delegata respondsToSelector:@selector(TABasedSouthwest_didSelectedAccessoryStatus:)]) {
                [self.delegata TABasedSouthwest_didSelectedAccessoryStatus:Y_StockChartTargetLineStatusAccessoryClose];
            }
        }
    }
}
...
-(void)item_click:(UIButton*)button{
    ...
    if (button.tag==Y_StockChartTargetLineStatusMA||button.tag==Y_StockChartTargetLineStatusBOLL) {
        ...
        if (self.delegata&&[self.delegata respondsToSelector:@selector(TABasedSouthwest_didSelectedStatus:)]) {
            [self.delegata TABasedSouthwest_didSelectedStatus:button.tag];
        }
        ...
        if (self.delegata&&[self.delegata respondsToSelector:@selector(TABasedSouthwest_didSelectedAccessoryStatus:)]) {
            [self.delegata TABasedSouthwest_didSelectedAccessoryStatus:button.tag];
        }
    }
}
#pragma mark 快速创建
