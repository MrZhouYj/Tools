...
#import "TAChartsConcerning.h"
#import "TAAmericaStudy.h"

@interface TAFeaturedNobody()
<UITableViewDelegate,
...
@implementation TAFeaturedNobody
...
-(void)didChange{
    ...
    _titleLabel.text =  CALanguages(@"股票");
    //    NSArray * segments = [CommonMethod readFromUserDefaults:CAMaretSortKey];
    NSMutableArray * segmentItems = @[].mutableCopy;
    [segmentItems addObject:CALanguages(@"自选")];
    [segmentItems addObject:CALanguages(@"股票")];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TAChartsConcerning * cell = [tableView dequeueReusableCellWithIdentifier:@"TAChartsConcerning"];
    if (_isShowSearchResult) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model) {
        if (self.delegata&&[self.delegata respondsToSelector:@selector(TAFeaturedNobody_didsectedMarket:)]) {
            [self.delegata TAFeaturedNobody_didsectedMarket:model];
...
-(void)searchWithKey:(NSString*)key{
    ...
    if (key&&key.length) {
        ...
        for (TAAmericaStudy *model in self.dataArray) {
...
-(UIView *)searchView{
    ...
    if (!_searchView) {
        ...
        searchTf.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[TAChartsConcerning class] forCellReuseIdentifier:@"TAChartsConcerning"];
    }
    return _tableView;
}
...
-(UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.dk_textColorPicker = DKColorPickerWithKey(BoldTextColor);
...
-(TALivesTogether *)segmentView{
    if (!_segmentView) {
        _segmentView = [[TALivesTogether alloc] initWithFrame:CGRectMake(0, 0, self.width, 40)];
...
-(void)reloadData{
    ...
    if (_allKeys.count>_currentIndex) {
        ...
        if (_currentIndex==0) {
            //自选
            self.dataArray = [TAAmericaStudy getSymbolsFav];
        }else{
            self.dataArray = [TAAmericaStudy getAllSymbols];
        }
