...
-(void)payClick:(CAPayButton*)btn{
    ...
    if (payWay.length) {
       ...
       if (self.delegata && [self.delegata respondsToSelector:@selector(TAHurtsSmiling_didSelected:)]) {
            [self.delegata TAHurtsSmiling_didSelected:payWay];
       }
       [self update];
    }
}
...
-(void)setSupportPayMethodArray:(NSArray<NSString *> *)supportPayMethodArray{
    ...
    for (NSString * pay  in supportPayMethodArray) {
        ...
        }else if ([pay isEqualToString:Wechat_pay]){
            self.weixinButton.hidden = NO;
            [self.buttonsArray addObject:self.weixinButton];
        }else if ([pay isEqualToString:Bank_card_pay]){
