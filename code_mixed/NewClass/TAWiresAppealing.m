...
#import "TALocksNoting.h"

#define     MAX_TEXTVIEW_HEIGHT         104

@interface TAWiresAppealing()
<UITextViewDelegate,
TALocksNotingDelegate>
{
...
@property (nonatomic, strong) TALocksNoting * moreView;

@end

@implementation TAWiresAppealing

-(TAPatroledExchanged *)picker{
    if (!_picker) {
        _picker  = [TAPatroledExchanged shareImgaePicker];
    }
    return _picker;
}
...
//        [self.contentView addSubview:self.moreButton];
        ...
        self.state = TAWiresAppealingNone;
    }
...
-(BOOL)resignFirstResponder{
    ...
    if (self.state==TAWiresAppealingMore) {
    ...
    }else if(self.state == TAWiresAppealingKeyBoard){
...
- (void)keyboardWillHide:(NSNotification *)notification{
    ...
    if (self.state == TAWiresAppealingKeyBoard) {
...
- (void)keyboardFrameWillChange:(NSNotification *)notification{
    ...
    self.state = TAWiresAppealingKeyBoard;
    [_moreButton setImage:IMAGE_NAMED(@"tupian") forState:UIControlStateNormal];
    [_moreButton setImage:IMAGE_NAMED(@"tupian_hightlight") forState:UIControlStateHighlighted];
}
...
-(void)TALocksNotingActionType_didSelectItem:(TALocksNotingActionType)type{
    switch (type) {
        case TALocksNotingActionTypeAlbum:
        {
            ...
            break;
        case TALocksNotingActionTypeCamera:
        {
...
-(void)moreButtonClick{
    ...
    switch (self.state) {
        ...
        
        {
            ...
            break;
        case TAWiresAppealingNone:
        {
             self.state = TAWiresAppealingMore;
...
-(TALocksNoting *)moreView{
    if (!_moreView) {
        _moreView = [TALocksNoting new];
