...
#import "TATouchedLeast.h"
#import "TASmileViral.h"
#import "TABeginsHappy.h"
...
-(TAStrongAsking *)currencyModel{
    if (!_currencyModel) {
        _currencyModel = [TAStrongAsking new];
    }
...
-(NSDictionary*)getKline{
    ...
    if (_currentKline) {
        [[TAVulnerablePlaces shareSocket] unsub:_currentKline];
    }
    _currentKline = [TAVulnerablePlaces getSub:dic];
...
-(void)sendMessageToSocket{
    ...
    [[TAVulnerablePlaces shareSocket] sendDataToSever:[self getRealTimeTrades]];
...
-(UITableView *)tableView{
    if (!_tableView) {
       ...
       [_tableView registerClass:[TABeginsHappy class] forCellReuseIdentifier:@"TABeginsHappy"];
       [_tableView registerClass:[TASmileViral class] forCellReuseIdentifier:@"TASmileViral"];
       [_tableView registerClass:[TATouchedLeast class] forCellReuseIdentifier:@"TATouchedLeast"];
       [_tableView registerClass:[DeepTableViewCell class] forCellReuseIdentifier:@"DeepTableViewCell"];
       [_tableView registerNib:[UINib nibWithNibName:@"TAChildCloud" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"TAChildCloud"];
...
-(void)setupDownMenuView{
    ...
    UIButton * sellBtn = [self private_creatButton:CALanguages(@"出售") color:[UIColor colorWithRGBHex:0xff5974]];
...
- (void)setupNavView
{
    ...
    label.dk_textColorPicker = DKColorPickerWithKey(BoldTextColor);
...
- (void)setuptopView
{
    ...
    rightTL.text = NSStringFormat(@"%@\n%@\n%@",CALanguages(@"高"),CALanguages(@"低"),CALanguages(@"成交额"));
...
-(void)loadCurrencyDetails{
    ...
    [TACrimeStudy GET:CAAPI_CURRENCY_DETAILS parameters:para success:^(id responseObject) {
        NSLog(@"币种简介:%@",responseObject);
...
//            return cell;
//        }else{
            TATouchedLeast * cell = [tableView dequeueReusableCellWithIdentifier:@"TATouchedLeast"];
...
//        }
        ...
        if (self.real_time_trades.count) {
            TASmileViral * cell = [tableView dequeueReusableCellWithIdentifier:@"TASmileViral"];
            if (indexPath.row==0) {
                [cell setTitleStyle:self.currentSymbolModel.ask_unit bid_code:self.currentSymbolModel.bid_unit];
            }else{
        ...
        TABeginsHappy * cell = [tableView dequeueReusableCellWithIdentifier:@"TABeginsHappy"];
        cell.enableCopy = NO;
...
-(TALivesTogether *)segmentView{
    if (!_segmentView) {
        _segmentView = [[TALivesTogether alloc] initWithFrame:CGRectMake(0, 0, MainWidth, 40)];
        _segmentView.segmentItems =@[CALanguages(@"盘口") ,CALanguages(@"成交"),CALanguages(@"简介")];
...
-(void)add_to_favorates{
    ...
    [TAAmericaStudy add_to_favorates:self.currentSymbolModel.market_id finshed:^(BOOL success) {
        if (success) {
            self.isfavorates=YES;
...
-(void)remove_from_favorates{
    ...
    [TAAmericaStudy remove_from_favorates:self.currentSymbolModel.market_id finshed:^(BOOL success) {
        if (success) {
            self.isfavorates=NO;
            [self.searchView reloadData];
        }
