...
#import "TADoingCombating.h"
#import "UIImage+Compress.h"

@interface TADoingCombating ()
...
- (void)viewDidLoad {
    ...
    self.navcTitle = @"实名认证";
    _user = [TAAppreciatePlans currentUser];
...
-(void)initAuditingFaliureEmptyView{
    ...
    _emptyView = [LYEmptyView emptyActionViewWithImage:IMAGE_NAMED(@"audit_faliure") titleStr:[TAAppreciatePlans currentUser].identity_state_name detailStr:nil btnTitleStr:CALanguages(@"重新认证") target:self action:@selector(againAction)];
...
-(void)initAuditingEmptyView{
    NSLog(@"初始化");
    _emptyView = [LYEmptyView emptyViewWithImage:IMAGE_NAMED(@"auditing") titleStr:[TAAppreciatePlans currentUser].identity_state_name detailStr:@""];
    _emptyView.titleLabTextColor = RGB(171, 175, 204);
    _emptyView.titleLabFont = FONT_REGULAR_SIZE(15);
    _emptyView.actionBtnFont = FONT_REGULAR_SIZE(15);
    _emptyView.actionBtnTitleColor = RGB(0, 108, 219);
...
-(void)initNotAuditSubViews{
    ...
    self.sendBtn = [TATheirOurselves buttonWithTitle:@"确认绑定"];
...
-(void)uploadImages{
    ...
    [SVProgressHUD show];
    [TACrimeStudy uploadImagesWithURL:CAAPI_MINE_UPLOAD_ID_CARD_FILES parameters:nil name:@[@"front",@"back",@"man"] images:@[_frontImage,_backImage,_holdFrontImage] progress:^(NSProgress *progress) {
