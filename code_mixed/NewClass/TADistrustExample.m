...
#import "TADistrustExample.h"
#import "TAImportanceSmiling.h"

@interface TADistrustExample ()

@property (nonatomic, strong) TAImportanceSmiling * emailTf;
@property (nonatomic, strong) TAImportanceSmiling * codeTf;
...
@implementation TADistrustExample
...
-(void)getEmailFromServer{
    ...
    [TACrimeStudy GET:CAAPI_MINE_EDIT_BIND_EMAIL parameters:nil success:^(id responseObject) {
...
-(void)sendBindEmailRequest{
    ...
    [TACrimeStudy POST:CAAPI_MINE_BIDN_EMAIL parameters:para success:^(id responseObject) {
        [SVProgressHUD dismiss];
...
-(void)initSubViews{
    ...
    self.emailTf = [TAImportanceSmiling showLoginTypeInputView];
    ...
    self.emailTf.contentType = TAImportanceSmilingContentEmailType;
    
    
    self.codeTf  = [TAImportanceSmiling showLoginTypeInputView];
...
-(void)routerEventWithName:(NSString *)eventName userInfo:(id)userInfo{
    ...
    if ([eventName isEqualToString:TAImportanceSmilingSendMessageSuccess]) {
...
-(void)unBindEmailClick{
    ...
    [SVProgressHUD show];
     ...
     [TACrimeStudy POST:CAAPI_MINE_UNBIDN_EMAIL parameters:nil success:^(id responseObject) {
         [SVProgressHUD dismiss];
         if ([responseObject[@"code"] integerValue]==20000) {
