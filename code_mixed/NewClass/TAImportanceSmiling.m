...
NSString * const TAImportanceSmilingSendMessageSuccess = @"TAImportanceSmilingSendMessageSuccess";


#import "TAImportanceSmiling.h"

@interface TAImportanceSmiling()
<UITextFieldDelegate>
...
@property (nonatomic, assign) TAImportanceSmilingType myType;
@property (nonatomic, assign) BOOL isUseInputTextWhileSendMessage;

@end

@implementation TAImportanceSmiling

+(instancetype)showWithType:(TAImportanceSmilingType)type{
    
    TAImportanceSmiling * input = [TAImportanceSmiling  new];
...
+(instancetype)showLoginTypeInputView{
    ...
    input.myType = TAImportanceSmilingLogin;
    
    return input;
}

-(void)setMyType:(TAImportanceSmilingType)myType{
    _myType = myType;
    [self creatUI];
...
-(instancetype)init{
    ...
    if (self) {
        ...
        self.minNumber = 0;
        self.contentType = TAImportanceSmilingContentNone;
...
-(void)creatUI{
    ...
    if (self.myType==TAImportanceSmilingLeftBigImageType) {
    ...
    self.inputView.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(UILabel *)chooseCountryLabel{
    if (!_chooseCountryLabel) {
        ...
        _chooseCountryLabel.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(void)layoutSubviews{
    ...
    if (self.myType==TAImportanceSmilingLeftNotiRightNotiType) {
    ...
    make.left.equalTo(_leftView&&!_leftView.isHidden?_leftView.mas_right:self.notiLabel.mas_left);
        ...
        }else if(self.myType==TAImportanceSmilingLeftNotiRightNotiType){
    ...
    if (_sendButton&&!_sendButton.isHidden) {
         ...
         if (self.inputView.text.length) {
             ...
             switch (self.contentType) {
                 case TAImportanceSmilingContentEmailType:
                     [self setSendEnable:[CommonMethod validateEmail:self.inputView.text]];
                     break;
                 case TAImportanceSmilingContentPhoneType:
...
-(void)sendButtonClick{
    ...
    if (self.needCheck) {
        [self routerEventWithName:@"TAImportanceSmiling_sendMessage" userInfo:nil];
    }else{
...
-(void)send_sms_validation_code:(NSDictionary*)result{
    ...
    if (self.country_code) {
     ...
     [TACrimeStudy POST:self.requestUrl parameters:mutPara success:^(id responseObject) {
         ...
         if ([responseObject[@"code"] integerValue]==20000){
             ...
             [self routerEventWithName:TAImportanceSmilingSendMessageSuccess userInfo:nil];
