...
@interface CAMenuCell : UITableViewCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        if ([[TASuburbsLives getCurrentSkinType] isEqualToString:DKThemeVersionNight]) {
//            #ededed  #212d42
            _separatorColor = HexRGB(0x212d42);
        }else{
            _separatorColor = HexRGB(0xededed);
        }
    }
    return self;
}
...
@interface CAMenuAction()
@property (nonatomic) NSString      *title;
@property (nonatomic) UIImage       *image;
@property (copy, nonatomic)void (^handler)(CAMenuAction *);
@end
@implementation CAMenuAction
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(CAMenuAction *))handler{
    CAMenuAction *action = [[CAMenuAction alloc] initWithTitle:CALanguages(title) image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(CAMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
@interface TACountryShare()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
{
    CGPoint          _refPoint;
    UIView          *_refView;
    CGFloat          _menuWidth;
...
@property(nonatomic,copy) NSArray<CAMenuAction *>   *actions;
...
static NSString *const menuCellID = @"CAMenuCell";
@implementation TACountryShare
...
+ (instancetype)menuWithActions:(NSArray<CAMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    ...
    TACountryShare *menu = [[TACountryShare alloc] initWithActions:actions width:width atPoint:point];
    return menu;
}
+ (instancetype)menuWithActions:(NSArray<CAMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    ...
    TACountryShare *menu = [[TACountryShare alloc] initWithActions:actions width:width relyonView:view];
    return menu;
}

- (instancetype)initWithActions:(NSArray<CAMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray<CAMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (CGPoint)getRefPoint{
    ...
    CGPoint refPoint;
    CGFloat menuHeight = (_actions.count > _maxDisplaCAount) ? _maxDisplaCAount * _menuCellHeight + kArrowHeight: _actions.count * _menuCellHeight + kArrowHeight;
    if (absoluteRect.origin.y + absoluteRect.size.height +  menuHeight > MainHeight - 10) {
...
- (void)calculateArrowAndFrame{
    ...
    width = _menuWidth;
    height = (_actions.count > _maxDisplaCAount) ? _maxDisplaCAount * _menuCellHeight + kArrowHeight: _actions.count * _menuCellHeight + kArrowHeight;
    // 默认在中间
    ...
    // 考虑向左右展示不全的情况，需要反向展示
    if (originX + width > MainWidth - 10) {
        originX = MainWidth - kDefaultMargin - width;
    }else if (originX < 10) {
    ...
    //如果不是根据关联视图，得算一次是否反向
    if (!_refView) {
        _isReverse = (originY + height > MainHeight - kDefaultMargin)?YES:NO;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    CAMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:menuCellID forIndexPath:indexPath];
    CAMenuAction *action = _actions[indexPath.row];
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        _tableView.bounces = _actions.count > _maxDisplaCAount? YES : NO;
        _tableView.rowHeight = _menuCellHeight;
        _tableView.tableFooterView = [UIView new];
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        [_tableView registerClass:[CAMenuCell class] forCellReuseIdentifier:menuCellID];
    }
    return _tableView;
}
- (UIView *)bgView{
...
//
////    NSLog(@" -=-=-=- %@",NSStringFromCGPoint([[event.allTouches anyObject] locationInView:kAppWindow]));
//
...
- (void)setMaxDisplaCAount:(NSInteger)maxDisplaCAount{
    if (_maxDisplaCAount == maxDisplaCAount)return;
    _maxDisplaCAount = maxDisplaCAount;
    _needReload = YES;
}
