...
+ (void)MATCHERPOST:(NSString *)URL
    ...
    NSString *Url = [URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:@"`#%^{}\"[]|\\<> "].invertedSet];
    NSLog(@"Url = %@",Url);
    _isAddDeviceInfo = false;
    parameters = [self appendDeviceInfo:parameters];
    _isAddDeviceInfo = true;
    NSString * mutString = @"";
    for (NSString * key in [parameters allKeys]) {
       mutString = [mutString stringByAppendingFormat:@"%@=%@&",key,parameters[key]];
    }
    NSLog(@"mutString = %@",mutString);
    mutString = [mutString substringToIndex:mutString.length-1];
    NSLog(@"mutString = %@",mutString);
    NSData *postData = [mutString dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
...
+(id)appendDeviceInfo:(id)parameters{
    ...
    //当前语言
    [mutDictionary setValue:[TAUrbanCooperation language] forKey:@"lang"];
    ...
    //获取当前时间戳
//    NSLog(@"%@",[TAAppreciatePlans currentUser]);
    if ([[TAAppreciatePlans currentUser] isAvaliable]) {
        [mutDictionary setValue:[TAAppreciatePlans currentUser].public_key forKey:@"public_key"];
...
+ (NSMutableDictionary*)appendSignature:(NSMutableDictionary*)para{
    ...
    mutString = [mutString stringByAppendingFormat:@"private_key=%@",[TAAppreciatePlans currentUser].private_key];
//    NSLog(@"拼接好的字符串 == %@\n",mutString);
