...
@implementation TAThoseSnowing

+ (BOOL)createTable
{
    //拿到数据库路径，创建数据库
    FMDatabase *db = [FMDatabase databaseWithPath:[TADramaticallySaving dbPath]];
    //开启数据库
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
...
/** 获取模型中的所有属性，并且添加一个主键字段pid。这些数据都存入一个字典中 */
...
- (BOOL)save
{
    ...
    TADramaticallySaving *ykDB = [TADramaticallySaving shareFMDBDatabase];
    __block BOOL res = NO;
...
{
    //判断是否是TAThoseSnowing的子类
    for (TAThoseSnowing *model in array) {
        if (![model isKindOfClass:[TAThoseSnowing class]]) {
            return NO;
        }
    }
    
    __block BOOL res = YES;
    TADramaticallySaving *jkDB = [TADramaticallySaving shareFMDBDatabase];
    // 如果要支持事务
...
#pragma mark - 删除数据.删除数据需要的资源：表名NSStringFromClass(self.class)，条件语句WHERE pid < 10
/** 通过条件删除数据 */
...
{
    //如果数组中的模型对象不是TAThoseSnowing的子类，那么不好意思，处理不了
    for (TAThoseSnowing *model in array) {
        if (![model isKindOfClass:[TAThoseSnowing class]]) {
            return NO;
        }
...
+ (NSArray *)findByCriteria:(NSString *)criteria
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            TAThoseSnowing *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
