...
@interface TADreamAutonomous ()
<
TAShoesShoesDelegate,
TALivesTogetherDelegate,
TAPersonGloballyDelegata,
...
/**
 我要买 我要卖
 */
@property (nonatomic, strong) TAPersonGlobally *menuView;
...
@implementation TADreamAutonomous
...
-(void)getList{
    ...
    [TACrimeStudy GET:CAAPI_OTC_LANDING parameters:nil success:^(id responseObject) {
        //刷新数据
        NSArray * currency = responseObject[@"data"][@"otc_currencies"];
        self.segmentItemsArray = [TAStrongAsking getModels:currency];
        [TAStrongAsking saveModels:currency byKey:@"otc_currencies"];
...
-(void)creatOrderClick:(NSString *)order_price
       ...
       originDictinary:(nonnull NSDictionary *)originDictionary
             orderView:(nonnull TAShoesShoes *)orderView{
    ...
    [SVProgressHUD show];
    [TACrimeStudy POST:CAAPI_OTC_CREATE_TRADING parameters:para success:^(id responseObject) {
        [SVProgressHUD dismiss];
...
-(void)freshListData{
    ...
    self.segmentView.segmentItems = [TALivesTogether getItemsFromArray:self.segmentItemsArray];
...
-(void)getData{
    ...
    TAStrongAsking * currency = self.segmentItemsArray[self.currentIndex];
    TAOtherInterested * tableView = (TAOtherInterested*)[self.pageView.childsViews objectAtIndex:self.currentIndex];
    ...
    [TACrimeStudy GET:CAAPI_OTC_GET_ADVERTISEMENTS parameters:para success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue]==20000) {
            ...
            if (responseObject[@"data"]) {
                NSMutableArray * currentData = tableView.dataSourceArray.mutableCopy;
                NSArray * models = [TASorrowPress getModels:responseObject[@"data"]];
...
-(void)routerEventWithName:(NSString *)eventName userInfo:(id)userInfo{
    ...
    if ([eventName isEqualToString:@"creatOrderAction"]) {
        ...
        _isLoadingData = YES;
        TASorrowPress * model = (TASorrowPress*)userInfo;
        NSDictionary * para = @{
            @"id":model.ID
        };
        
        [SVProgressHUD show];
        [TACrimeStudy GET:CAAPI_OTC_IS_ADVERTISEMENT_CANCELLED parameters:para success:^(id responseObject) {
            [SVProgressHUD dismiss];
            ...
            if ([responseObject[@"code"] integerValue]==20000) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [TAAllegedlyCause showAlertWithTitle:responseObject[@"message"] message:@"" completionBlock:^(NSUInteger buttonIndex, TAAllegedlyCause * _Nonnull alertView) {
...
-(void)showCreatOrderView:(NSDictionary*)originDictionary{
    ...
    TAShoesShoes * creatOrderView = [[TAShoesShoes alloc] initWithFrame:CGRectMake(0, MainHeight-height, MainWidth, height)];
...
-(void)initSubViews{
    ...
    self.menuView = [TAPersonGlobally new];
    ...
    self.segmentView = [[TALivesTogether alloc] initWithFrame:CGRectMake(0, 0, MainWidth, 33)];
...
-(void)showMoreAction:(UIButton*)btn{
    ...
    TACountryShare * menuView = [TACountryShare menuWithActions:@[action1,action2] width:150 relyonView:btn];
    [menuView show];
}
...
-(void)jumpToNextController:(NSInteger)type{
    if (type==2) {
        TAChanceRound * order = [TAChanceRound new];
        order.type = 2;
        [self.navigationController pushViewController:order animated:YES];
    }else if (type==1){
        TAChanceRound * order = [TAChanceRound new];
...
-(void)initPageContentView{
    ...
    for (int i=0; i<self.segmentItemsArray.count; i++) {
        TAOtherInterested * tableView = [TAOtherInterested new];
