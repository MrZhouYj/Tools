...
-(void)setModel:(TATrialsRelief *)model{
    _model = model;
    ...
    if ([model.mode_of_payment isEqualToString:Bank_card_pay]) {
        self.payWayImageView.image = IMAGE_NAMED(@"Bankcard");
    }else if ([model.mode_of_payment isEqualToString:Alipay]){
        self.payWayImageView.image = IMAGE_NAMED(@"Alipay");
    }else if ([model.mode_of_payment isEqualToString:Wechat_pay]){
