...
@property (nonatomic, strong) TAIndeedPlenty * bidView;

@property (nonatomic, strong) TAIndeedPlenty * askView;
//放 卖盘 买盘 centerview
...
@property (nonatomic, strong) TAShowingIndeed * changeDeepView;
...
@implementation TAPublicHowever
...
//        [TAAreasCalled showActionSheet:array selectIndex:_curIndex click:^(NSInteger index) {
//            
...
#pragma mark 底部按钮 Start
    ...
    TAShowingIndeed * changeDeepView = [TAShowingIndeed new];
...
#pragma mark 设置深度图 买 卖
    self.askView = [TAIndeedPlenty new];
    [self.centerContentView addSubview:self.askView];
    
    self.bidView = [TAIndeedPlenty new];
