...
@implementation TAPointedWonderful
...
-(instancetype)init{
    ...
    if (self) {
        ...
        if (self.delegata&&[self.delegata respondsToSelector:@selector(TAPointedWonderful_didSelectIndex:)]) {
            [self.delegata TAPointedWonderful_didSelectIndex:0];
        }
...
-(void)didChooseIndex:(NSInteger)index{
    ...
    if (self.delegata&&[self.delegata respondsToSelector:@selector(TAPointedWonderful_didSelectIndex:)]) {
        [self.delegata TAPointedWonderful_didSelectIndex:index];
    }
...
-(void)setDefaultAttribute:(UILabel*)label{
    ...
    label.dk_textColorPicker = DKColorPickerWithKey(NormalgrayColor);
}
