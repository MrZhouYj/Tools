...
#import "TAAdvertisementDispirited.h"
#import "TAShowingIndeed.h"
#import "TARecentInternational.h"
#import "TAAllegedlyCause.h"

#define lowColor HexRGB(0xe2e2ea)
#define highColor HexRGB(0xc7c8d7)

@interface TALargeThere()
<UITextFieldDelegate>
{
    TARecentInternational * _buyMenuView;
    TARecentInternational * _sellMenuView;
...
@property (nonatomic, strong) TAAdvertisementDispirited *progressView;
...
-(TAAdvertisementDispirited *)progressView{
    if (!_progressView) {
        _progressView = [TAAdvertisementDispirited new];
...
-(void)languageDidChange{
    ...
    [_sellMenuView setTitle:CALanguages(@"出售")];
...
-(void)initTitleMenuView{
    ...
    _buyMenuView = [TARecentInternational new];
    _sellMenuView = [TARecentInternational new];
...
-(void)routerEventWithName:(NSString *)eventName userInfo:(id)userInfo{
    ...
    if ([eventName isEqualToString:NSStringFromClass(TARecentInternational.class)]) {
...
-(void)changeNumberBanlanceDisplay{
    ...
    if ([TAAppreciatePlans currentUser].isAvaliable&&(self.bid_currency_amount.length||self.ask_currency_amount.length)) {
...
-(NSString *)caculateMax{
    ...
    float bid_fee = self.bid_fee.floatValue;
    if (bid_fee<=0) {
        bid_fee = 0;
    }
   
    NSDecimalNumber * unitPriceDecimal = [NSDecimalNumber decimalNumberWithString:[NSString stringWithFormat:@"%f",unitPrice.floatValue*(1+bid_fee)]];
    ...
    @try {
        ...
        max = [self Rounding:result afterPoint:self.ask_currency_scale];
...
-(void)judgeLogin{
    ...
    if (user.isAvaliable) {
        ...
        if (_buyMenuView.isSelect) {
            ...
            [_loginButton setTitle:NSStringFormat(@"%@ %@",CALanguages(@"出售"),[self.ask_code uppercaseString]) forState:UIControlStateNormal];
            self.tradeType = TradingSell;
        }
        if (self.is_off) {
            _loginButton.enabled = false;
            [_loginButton setTitle:@"休市中" forState:UIControlStateNormal];
            _loginButton.backgroundColor = [UIColor groupTableViewBackgroundColor];
            [_loginButton setTitleColor:RGB(145,146,177) forState:UIControlStateNormal];
        }
...
-(void)adjustNumberInput{
    NSString * result = _numberTextField.text;
    int count = [self.number_of_units intValue];
    if (count<=0) {
        count = 100;
    }
    int num = [result intValue] / count * count;
    _numberTextField.text = [NSString stringWithFormat:@"%d",num];
}
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (pointRange.location != NSNotFound) {
        ...
        if (textField==_priceTextField) {
            NSLog(@"precision %@",self.precision);
            pointAfterMaxLength = [self.precision[@"bid_currency_scale"] intValue];//ask 最多小数点后的位数
        }else{
...
- (void)changeBusinessVolume{
    ...
    if (price.length&&number.length) {
        if (number.floatValue>0&&price.floatValue>0) {
...
-(void)buyOrSellAction{
    ...
    if (![TAAppreciatePlans currentUser].is_agent) {
        [TAAllegedlyCause showAlertWithTitle:CALanguages(@"请先完善信息") message:@"" completionBlock:^(NSUInteger buttonIndex, TAAllegedlyCause * _Nonnull alertView) {
            if (buttonIndex==1) {
                [self.delegata gotoAgentController];
            }
            
        } cancelButtonTitle:CALanguages(@"取消")  otherButtonTitles:CALanguages(@"去完善"),nil];
        return;
    }
    [self adjustNumberInput];
    NSString * price = _priceTextField.text;
    ...
    [SVProgressHUD show];
    NSDictionary * para = @{
        @"is_ask": self.tradeType==TradingBuy?@"false":@"true",
    ...
    [TACrimeStudy MATCHERPOST:[NSString stringWithFormat:@"%@%@",self.market_id,CAAPI_CRYPTO_TO_CRYPTO_CREATE_ORDER] parameters:para success:^(id responseObject) {
