...
#import "TAThereComplex.h"

@interface TATheseTakes ()
<UITableViewDelegate,
...
@property (nonatomic, strong) TAThereComplex * headView;

@property (nonatomic, strong) UIView * bottomView;

@end

@implementation TATheseTakes
...
-(UIView*)creatView:(NSString*)title image:(NSString*)imageUrl{
    ...
    label.dk_textColorPicker = DKColorPickerWithKey(NormalBlackColor);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row<self.dataArray.count) {
        ...
        if (dic[@"controller"]) {
            ...
            if ([dic[@"isNeesLogin"] boolValue]&&![TAAppreciatePlans currentUser].isAvaliable) {
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[TAOutsidersParty class] forCellReuseIdentifier:@"TAOutsidersParty"];
...
-(TAThereComplex *)headView{
    if (!_headView) {
        _headView = [[TAThereComplex alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame),150+kStatusBarHeight)];
    }
    return _headView;
}
...
-(NSArray *)dataArray{
    if (!_dataArray) {
        NSString * agent_c = @"TAAboutEntertain";
        TAAppreciatePlans * user = [TAAppreciatePlans currentUser];
        if (user.is_agent) {
            agent_c = @"TAGoodsEvening";
        }
        
        _dataArray = @[
            @{@"image":@"wallet",@"text":@"我的资产",@"controller":@"TATakenTibet",@"isNeesLogin":@(YES)},
            @{@"image":@"transfer",@"text":@"资金划转",@"controller":@"TAChancesRemarks",@"isNeesLogin":@(YES)},
            @{@"image":@"Receivables",@"text":@"收款方式",@"controller":@"TAGoingSnowman",@"isNeesLogin":@(YES)},
            @{@"image":@"ID",@"text":@"实名认证",@"controller":@"FaceidViewController",@"isNeesLogin":@(YES)},
            
            @{@"image":@"Mobile phone",@"text":@"绑定手机",@"controller":@"TAHappenedExcellent",@"isNeesLogin":@(YES)},
            @{@"image":@"information",@"text":@"完善信息",@"controller":agent_c,@"isNeesLogin":@(YES)},
            @{@"image":@"Mailbox",@"text":@"邮箱设置",@"controller":@"TADistrustExample",@"isNeesLogin":@(YES)},
            
            //  @{@"image":@"passwordimage",@"text":@"资金密码",@"controller":@"TAPsychicScientific",@"isNeesLogin":@(YES)},CAFeedBackViewController
            //                       @{@"image":@"gift",@"text":@"邀请码",@"controller":@"TAAtmosphereMindful",@"isNeesLogin":@(YES)},
            //                       @{@"image":@"friends",@"text":@"我的好友",@"controller":@"TAThinkWorry",@"isNeesLogin":@(YES)},
            
            @{@"image":@"Setup",@"text":@"设置",@"controller":@"TAAmericanEpidemics",@"isNeesLogin":@(NO)},
        ];
    }
    return _dataArray;
}
